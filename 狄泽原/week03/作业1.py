...
class TorchModel(nn.Module):
    def __init__(self, vector_dim, sentence_length, vocab , hidden_size):
        ...
        self.rnn = nn.RNN(input_size=vector_dim, hidden_size=hidden_size, batch_first=True , bidirectional=False)
...
def evaluate(model, vocab, sample_length):
    ...
    no_target_count = sum(1 for pos in y if pos == -1)
    print(f"本次预测集中共有{200 - no_target_count}个含特定字符样本，{no_target_count}个无特定字符样本")
    correct, total = 0, 0
    with torch.no_grad():
        y_pred = model(x)
        y_pred = y_pred - 1 #模型预测
    ...
    print(f"测试集准确率：{acc:.4f}，正确预测：{correct}/{total}")
    return acc
...
def predict(model_path, vocab_path, input_strings):
    ...
    with torch.no_grad():  #不计算梯度
        ...
        result_pos = [p.item()-1 for p in result]
