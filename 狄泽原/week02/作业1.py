...
class TorchModel(nn.Module):
    def __init__(self, input_size , num_classes):
        ...
        self.activation = nn.Softmax(dim=1)  # nn.Sigmoid() sigmoid归一化函数
...
def evaluate(model):
    ...
    print(f"本次预测集中各类别的数量：{np.bincount(y.numpy())}" )
    correct =  0
    with torch.no_grad():
        y_pred_probs = model(x)  # 模型预测 model.forward(x)
...
def main():
    ...
    # 保存模型
    torch.save(model.state_dict(), "five_class_model.bin")
    # 画图
    print(log)
