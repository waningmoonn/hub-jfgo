...
class TorchModel(nn.Module):
    def __init__(self, input_size, hidden_size, output_size):
        ...
        self.linear2 = nn.Linear(hidden_size, output_size)  # 线性层
        # self.activation = torch.nn.Softmax(dim=1)  # nn.Sigmoid() sigmoid归一化函数
        self.loss = nn.CrossEntropyLoss()  # loss函数采用均方差损失

    # 当输入真实标签，返回loss值；无真实标签，返回预测值
    def forward(self, x, y=None):
        # print('x: ', x)
        y_linear1 = self.linear(x)  # (batch_size, input_size) -> (batch_size, 1)
        y_linear1_relu = F.tanh(y_linear1)  # (batch_size, hidden_size) -> (batch_size, output_size)
        y_pred = self.linear2(y_linear1_relu)  # (batch_size, hidden_size) -> (batch_size, output_size)
