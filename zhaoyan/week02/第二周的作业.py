...
class TorchModel(nn.Module):
    def __init__(self, input_size):
        ...
        # self.activation = torch.softmax  # 分类softmax函数
        self.loss = nn.CrossEntropyLoss()# loss函数采用交叉熵

    # 当输入真实标签，返回loss值；无真实标签，返回预测值
    def forward(self, x, y=None):
        x = self.linear(x)  # (batch_size, input_size) -> (batch_size, 1)
        y_pred = torch.softmax(x, dim=1)  # (batch_size, 5) -> (batch_size, 5)
...
# 随机生成一个5维向量，如果某个值最大，标签再在向量的位置
def build_sample():
    x = np.random.random(5)
    max_position = np.argmax(x)
    # print(x,max_position)
    return x, max_position
...
def build_dataset(num_samples):
    ...
    Y_tensor = torch.LongTensor(np.array(Y))  # 注意：标签用LongTensor
