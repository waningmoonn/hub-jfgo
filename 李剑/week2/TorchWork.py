...
def build_dataset(total_sample_num):
    ...
    for i in range(total_sample_num):
        ...
        X.append(x)
        Y.append(y)
    # print("999999999",np.array(X))
    # print("111111111",np.array(Y))
...
def evaluate(model):
    ...
    print("本次预测集中共有%d个一类样本，%d个二类样本，%d个三类样本，%d个四类样本，%d个五类样本" % (count0, count1, count2, count3, count4))
    correct, wrong = 0, 0
    with torch.no_grad():
        y_pred = model(x)  # 模型预测 model.forward(x)
        predicted_classes = torch.argmax(y_pred, dim=1)  # 获取每个样本的预测类别
...
def predict(model_path, input_vec):
    ...
    for vec, res in zip(input_vec, result):
        # 获取预测类别（最高分的索引）
        pred_class = torch.argmax(res).item() + 1
        # 获取最大分数值
        pred_prob = torch.max(res).item()
        print("输入：%s, 预测类别：%d, 分数值：%f" % (vec, pred_class, pred_prob))  # 打印结果
