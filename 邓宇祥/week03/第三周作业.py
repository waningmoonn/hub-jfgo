...
class TorchModel(nn.Module):
    def __init__(self, vector_dim, sentence_length, vocab,hidden_size=8):
    ...
    def forward(self, x, y=None):
        ...
        # 将嵌入向量输入到rnn层，得到输出以及最终的隐藏状态hn
        output, hn = self.rnn(x)                         #x为（batch_size, sen_len,hidden_size）
        x = hn.squeeze(0)


        y_pred = self.classify(x)   ##(batch_size, sen_len, num_classes)
...
def predict(model_path, vocab_path, input_strings):
    ...
    for i, input_string in enumerate(input_strings):
        ...
        probability = result[i][predicted_class].item()

        print("输入：%s, 预测类别：%d, 概率值：%f" % (input_string, predicted_class,probability)) #打印结果
...
if __name__ == "__main__":
    main()
    test_strings = ["fnvf我e", "wz你dfg", "rqwdeg", "nk我www"]
