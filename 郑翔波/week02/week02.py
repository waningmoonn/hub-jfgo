...
规律：x是一个5维向量，用交叉熵实现一个多分类任务，五维随机向量最大的数字在哪维就属于哪一类
...
class TorchModel(nn.Module):
    def __init__(self, input_size):
        ...
        #self.activation = torch.softmax  # 用softmax替换sigmoid
...
def build_sample():
    ...
    #y = x.index(max_value)
    #y = np.argmax(x)
    y = np.zeros_like(x) #不知道怎么实现，网上找的
    y[np.argmax(x)] = 1 #不知道怎么实现，网上找的
    return x, y
...
def evaluate(model):
    ...
    with torch.no_grad():
        ...
        for y_p, y_t in zip(y_pred, y):  # 与真实标签进行对比
            if np.argmax(y_p) == np.argmax(y_t): #为什么不能用torch.argmax(y_p) 
                correct += 1  # 负样本判断正确
            else:
                wrong += 1
